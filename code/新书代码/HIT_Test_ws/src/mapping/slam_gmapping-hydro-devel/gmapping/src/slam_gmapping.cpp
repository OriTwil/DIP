...
void SlamGMapping::startLiveSlam()
{
  ...
  //scan_filter_记录转换到odom坐标系后的激光雷达数据
...
{
  //得到激光雷达相对于车身坐标系(base_footprint)的位姿
  ...
  //在这里激光如果是-1.57～1.57(-90到正50)，则theta为-1.57
...
//这表示带方向位姿的点
  ...
  /*激光必须被称为“闪光”。我们传入计算的角度增量的绝对值 GMapping需要正角度增量的假设。如果
实际增量是负的，我们之前会交换范围的顺序 将每次扫描输入GMapping*/
  // 根据上面得到的激光雷达的数据信息 来初始化一个激光传感器
  //gsp_laser_初始化后保存每一条激光线对应的角度和最大测量范围
...
SlamGMapping::addScan(const sensor_msgs::LaserScan& scan, GMapping::OrientedPoint& gmap_pose)
{
  ...
  } else 
  {
    ...
    {
      // Must filter out short readings, because the mapper won't
      //小于最小激光距离0.08时，都变成最大距离,即返回的是0时，也处理成最大值了
