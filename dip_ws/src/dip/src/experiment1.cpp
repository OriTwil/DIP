...
int main(int argc, char **argv)
{
	...
	while (ros::ok())
	{
		...
		Mat gray_img, equalized_img;
		cvtColor(src_frame, gray_img, COLOR_BGR2GRAY); //将彩色图像转换为灰度图像
		equalizeHist(gray_img, equalized_img); //直方图均衡化
		imshow("equalized", equalized_img); //显示直方图均衡化后的图像
