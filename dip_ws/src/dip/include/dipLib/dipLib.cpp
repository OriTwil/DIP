...
void HCharSeg::load(Mat src) {
    ...
    /* 2. Color Segment */
    Mat bin = segment(src);
    cvtColor(bin, bin, cv::COLOR_GRAY2BGR);
    imshow("Color_Segment", bin&src);
    cvtColor(bin, bin, cv::COLOR_BGR2GRAY);

    /* 3. select Biggest Contours */
    vector<vector<Point>> contours; //定义轮廓集合
    vector<Vec4i> hierarchy;

    findContours(bin, contours, hierarchy, cv::RETR_EXTERNAL, cv::CHAIN_APPROX_SIMPLE);//CV_RETR_EXTERNAL只检测外部轮廓
...
#define USE_CONVEX_HULL 1
    ...
    contours.clear();
    hierarchy.clear();
    cvtColor(hArea, hArea, cv::COLOR_GRAY2BGR);
    findContours(hArea, contours, hierarchy, cv::RETR_EXTERNAL, CHAIN_APPROX_SIMPLE);//CV_RETR_EXTERNAL只检测外部轮廓
    ...
    contours.clear();
    hierarchy.clear();
    findContours(deltaMat, contours, hierarchy, cv::RETR_EXTERNAL, cv::CHAIN_APPROX_NONE);//CV_RETR_EXTERNAL只检测外部轮廓
    ...
    if(deltaList.size() >= 2){
        cvtColor(deltaMat, deltaMat, cv::COLOR_GRAY2BGR);
